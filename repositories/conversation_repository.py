...
class ConversationRepository(BaseRepository):
    ...
    async def get_conversation(self, conversation_id: str) -> Optional[Conversation]:
        ...
        query = """
            SELECT id, target_id, target_type, active_model, active_provider, created_at, updated_at
    ...
    async def save_conversation(self, conv: Conversation) -> None:
        ...
        """
        query = """
            INSERT INTO ai_conversations (id, target_id, target_type, active_model, active_provider, updated_at)
        ...
        await self.db.execute(query, (
            conv.id, conv.target_id, conv.target_type, conv.active_model, conv.active_provider
        ))
    ...
    async def add_message(self, conversation_id: str, message: Message) -> None:
        """Append a single message record to the conversation.
        ...
        """
        await self.db.execute(query, (
            conversation_id, message.role, message.content, message.prompt_tokens, message.completion_tokens
        ))
    ...
    async def trim_messages(self, conversation_id: str, max_messages: int) -> int:
        ...
        """
        # Load messages IDs ordered by created_at desc
        query = """
